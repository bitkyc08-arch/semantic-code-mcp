...
export class MilvusCache {
  ...
  async reconcileOrphans() {
    ...
    // 1. Paginated query: collect all unique file paths + IDs
    while (true) {
      ...
      for (const row of rows) {
        if (!pathToIds.has(row.file)) pathToIds.set(row.file, []);
        pathToIds.get(row.file).push(row.id);
      }
    ...
    const orphanPaths = [];
    let orphanCount = 0;
    for (const [filePath, ids] of pathToIds) {
      try {
    ...
    // 3. Delete orphans via filter (reuse existing escapeFilterString)
    if (orphanPaths.length > 0) {
      let failed = 0;
      for (const op of orphanPaths) {
        const escaped = escapeFilterString(op);
        try {
          await this.client.delete({
            collection_name: this.collectionName,
            filter: `file == "${escaped}"`,
          ...
          failed++;
          console.error(`[Cache] Reconcile delete failed: ${op}: ${err.message}`);
        }
      }
      const removed = orphanPaths.length - failed;
      console.error(
        `[Cache] Reconciled ${removed} orphan paths (${orphanCount} chunks)` +
