...
export async function loadConfig(workspaceDir = null) {
  try {
    ...
    if (workspaceDir) {
      ...
      config.cacheDirectory = path.join(baseDir, ".semantic-code-cache");
    } else {
  ...
  if (process.env.SMART_CODING_DEDUP_MAX_PER_FILE !== undefined) {
    const value = parseInt(process.env.SMART_CODING_DEDUP_MAX_PER_FILE, 10);
    if (!isNaN(value) && value >= 0 && value <= 100) {
      config.dedupMaxPerFile = value;
      console.error(`[Config] Dedup max per file: ${value}`);
    } else {
      console.error(`[Config] Invalid SMART_CODING_DEDUP_MAX_PER_FILE: ${process.env.SMART_CODING_DEDUP_MAX_PER_FILE}, using default`);
    }
  }
