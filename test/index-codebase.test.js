...
describe('Index Codebase Tool Handler', () => {
  ...
  describe('Tool Handler', () => {
    it('should return accepted response immediately (non-blocking)', async () => {
    ...
    it('should reject concurrent calls with progress info', async () => {
      // Start first indexing
      await clearTestCache(fixtures.config);
      fixtures.cache.setVectorStore([]);
      fixtures.cache.fileHashes = new Map();

      // Start background indexing via handler
      await IndexCodebaseFeature.handleToolCall(
        createMockRequest('b_index_codebase', { force: true }),
        fixtures.indexer
      );

      // Wait for isIndexing to become true
      await new Promise(resolve => setTimeout(resolve, 100));
    ...
    it('should accept force parameter and return immediately', async () => {
      const request = createMockRequest('b_index_codebase', { force: true });
      const result = await IndexCodebaseFeature.handleToolCall(request, fixtures.indexer);
      const parsed = JSON.parse(result.content[0].text);
